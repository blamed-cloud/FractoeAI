			...
			if self.is_max:
				...
				for child_state in self.children:
					...
					if self.depth_limit == 5:						######
						print("child is: ", str(child_state), file=sys.stderr)		######
						print("child value is: ", str(child_value), file=sys.stderr)	######
						print("best value is : ", str(self.value), file=sys.stderr)	######
					if child_value > self.value:
						if self.depth_limit == 5:					######
							print("### new best child found ###", file=sys.stderr)	######
						self.best_child = [child_state]
					elif child_value == self.value:
						if self.depth_limit == 5:					######
							print("### another best child added ###", file=sys.stderr)######
						self.best_child += [child_state]
